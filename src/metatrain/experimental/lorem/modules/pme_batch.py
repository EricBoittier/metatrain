...
jax-pme ``batched_tiled`` quantizes PBC atom counts to ``BM`` and the k-grid
to ``BK``. torch-pme evaluates one ``System`` at a time (a Python list batch).
These helpers expose the same padding arithmetic and PBC split so a later
packed kernel can match iris without changing today's per-system potentials.
"""
...
def pad_to_tile(count: int, tile: int) -> int:
    """Round ``count`` up to a multiple of ``tile`` (iris ``BM`` / ``BK``)."""
    if tile <= 0:
