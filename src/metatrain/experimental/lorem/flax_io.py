...
module does **not** depend on JAX: reading msgpack uses flax or marathon when
...
def read_flax_msgpack(path: str | Path) -> Dict[str, Any]:
    ...
    try:
        ...
        return read_msgpack(path)
    except ImportError as err:
        raise ImportError(
            "Reading flax checkpoints needs flax or marathon in a JAX env. "
            "Do not install those into the shared metatrain venv."
        ) from err
