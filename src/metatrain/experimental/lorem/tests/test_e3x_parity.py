...
Matching ``iris-infra``'s ``test_petjax_parity.py`` in spirit: load a
checkpoint dumped from the reference implementation, run both on identical
inputs, assert the outputs agree -- adapted here to not require the
reference framework (JAX/e3x) to be installed at test time, since the
fixture is dumped ahead of time instead of generated on the fly.
"""
...
class TestTensorDenseParity:
    ...
    def torch_output(self, data, max_degree):
        ...
        with torch.no_grad():
            for l, key in enumerate(("dense_0", "dense_1", "dense_2")):
                # e3x Dense kernel is (in, out); torch Linear.weight is (out, in).
                td.dense.layers[l].weight.copy_(torch.from_numpy(data[key].T).float())
...
def test_tensor_product_matches_e3x_with_nontrivial_phase():
    ...
    tp = TensorProduct(
        left_max_degree=max_degree, right_max_degree=max_degree, out_max_degree=0,
        include_pseudotensors=False, n_features=2,
    )
    with torch.no_grad():
    ...
    left = e3x_compat.from_e3x_convention(torch.from_numpy(data["left"]).float(), max_degree)
    right = e3x_compat.from_e3x_convention(torch.from_numpy(data["right"]).float(), max_degree)
